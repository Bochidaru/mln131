...
<body><main>
  ...
  <section class="panel"><h2>Người chơi & bảng điểm</h2><table><thead><tr><th>Tên</th><th>Điểm</th><th>Khu vực</th><th>Trạng thái</th><th>Vai trò</th><th>ID</th><th>Điều khiển</th></tr></thead><tbody id="players"></tbody></table><p id="notice"></p></section>
...
async function guide(id,name,enabled){const r=await fetch('/admin/api/players/'+encodeURIComponent(id)+'/guide?enabled='+(!enabled),{method:'POST'});document.querySelector('#notice').textContent=r.ok?(enabled?'Đã gỡ hướng dẫn viên của ':'Đã đặt hướng dẫn viên: ')+name+'.':'Không thể đổi vai trò người chơi này.';await load()}
async function award(id,name,input){const amount=Number.parseInt(input.value,10);if(!Number.isInteger(amount)||amount<=0){document.querySelector('#notice').textContent='Số điểm tặng phải là số nguyên dương.';input.focus();return}const r=await fetch('/admin/api/players/'+encodeURIComponent(id)+'/points?amount='+amount,{method:'POST'});document.querySelector('#notice').textContent=r.ok?'Đã tặng '+amount+' điểm cho '+name+'.':'Không thể tặng điểm cho người chơi này.';if(r.ok)pointDrafts.set(id,amount);await load()}
function awardControl(x){const wrap=document.createElement('div');wrap.className='award';const input=document.createElement('input');input.type='number';input.min='1';input.step='1';input.value=String(pointDrafts.get(x.id)??5);input.setAttribute('aria-label','Điểm tặng cho '+x.name);input.oninput=()=>pointDrafts.set(x.id,input.value);const button=document.createElement('button');button.className='award-button';button.textContent='Tặng điểm';button.onclick=()=>award(x.id,x.name,input);wrap.append(input,button);return wrap}
...
async function load(){try{const r=await fetch('/admin/api/status',{cache:'no-store'});if(!r.ok)throw new Error();const s=await r.json();online.textContent=s.onlineCount;duels.textContent=s.duels.length;cpu.textContent=s.metrics.cpuPercent==null?'Đang đo':s.metrics.cpuPercent+'%';ram.textContent=bytes(s.metrics.availableMemoryBytes);updated.textContent='CẬP NHẬT '+new Date().toLocaleTimeString('vi-VN');const p=document.querySelector('#players');p.replaceChildren();if(!s.players.length)empty(p,7,'Chưa có người chơi online.');else s.players.forEach(x=>{const controls=document.createElement('div');controls.className='actions';const g=document.createElement('button');g.className='guide'+(x.isGuide?' is-guide':'');g.textContent=x.isGuide?'Gỡ hướng dẫn':'Hướng dẫn viên';g.onclick=()=>guide(x.id,x.name,x.isGuide);const b=document.createElement('button');b.textContent='Kick';b.onclick=()=>kick(x.id,x.name);controls.append(g,awardControl(x),b);p.append(row([x.name,x.score,x.area,x.inDuel?'Đang duel':'Đang tham quan',x.isGuide?'Hướng dẫn viên':'Khách tham quan',x.id.slice(0,8),controls]))});const d=document.querySelector('#duel-list');d.replaceChildren();if(!s.duels.length)empty(d,4,'Không có trận duel nào.');else s.duels.forEach(x=>d.append(row([x.first.name,x.second.name,x.first.wins+' : '+x.second.wins,x.finishing?'Đang trả về':'Đang đấu'])))}catch{document.querySelector('#notice').textContent='Không tải được dữ liệu dashboard.'}}
