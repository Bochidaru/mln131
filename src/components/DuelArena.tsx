...
function Gun() {
  ...
  useEffect(() => {
    const gunModel = gun.current
    if (!gunModel) return
    camera.add(gunModel)
    return () => { camera.remove(gunModel) }
  }, [camera])
  useFrame((state) => { if (gun.current) gun.current.rotation.z = Math.sin(state.clock.elapsedTime * 2) * 0.015 })
  return <group ref={gun} position={[0.34, -0.28, -0.62]} rotation={[-0.08, Math.PI, 0]}>
...
export function DuelArena() {
  ...
  return <group>
    ...
    <group position={[ARENA, 0, ARENA]}>
      ...
      {opponent && <group position={[opponent.x - ARENA, 0, opponent.z - ARENA]} rotation={[0, Math.atan2(opponent.dirX, opponent.dirZ), 0]}><mesh position={[0, .85, 0]}><capsuleGeometry args={[.3, .9, 5, 10]} /><meshStandardMaterial color="#a83f37" /></mesh><mesh position={[0, 1.55, 0]}><sphereGeometry args={[.23, 12, 9]} /><meshStandardMaterial color="#d8a078" /></mesh></group>}
    </group>
  </group>
}
