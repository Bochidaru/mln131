...
function GalleryRoom({ layout }: { layout: GalleryLayout }) {
  ...
  const offsets =
    room.posters.length === 1
      ? [0]
      : room.posters.length === 2
        ? [-2.7, 2.7]
        : [-4.6, 0, 4.6];
    ...
    <group>
      ...
      {room.posters.map((item, index) => (
        <Poster
          ...
          data={item}
          position={[
            outerX + (side === "left" ? 0.19 : -0.19),
            2.48,
            center.z + offsets[index],
          ]}
      ...
      <mesh position={[side === "left" ? -17.8 : 17.8, 5.2, center.z]}>
        <boxGeometry args={[0.12, 0.12, size.depth - 1.2]} />
        <meshStandardMaterial
          color={charcoal}
          metalness={0.62}
          roughness={0.34}
